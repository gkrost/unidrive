...
|---|---|---|---|---|
| `:app:core` | 2026-05-13; fully resolved 2026-05-14 (UD-810: 246769d; UD-811: 6556992 + 9a55b6c) | 9 | 3 inline + 9 via UD-810/UD-811 | — |
| `:app:sync` | — | — | — | — |
| `:app:cli` | — | — | — | — |
| `:app:benchmark` | — | — | — | — |
| `:app:e2e-360` | — | — | — | — |
...
| `HttpRetryBudgetMatrixTest.kt` | `429 honors Retry-After capped by maxRetryAfter` | misleading | A 429 below the storm threshold does not open the circuit; a storm of 429s makes `resumeAfterEpochMs` honor the largest observed `Retry-After`. | Resolved by 6556992. Renamed to `429 storm opens circuit honoring largest Retry-After observed`. The vapor `maxRetryAfter` cap is acknowledged in the comment; if implemented, a new test pins it separately. |
| `HttpRetryBudgetMatrixTest.kt` | `network IOException retries with exponential backoff` | misleading | `isRetriableIoException` classifies transient TCP failures as retriable and DNS/SSL misconfig as non-retriable. | Resolved by 6556992. Renamed to `isRetriableIoException distinguishes transient TCP failures from misconfig`. Backoff verification lives at the provider layer. |
| `InFlightDedupTest.kt` | `concurrent callers for same key share exactly one loader invocation` / `loader failure is rethrown to all callers …` | impl-anchored (timing) | Concurrent callers share one loader invocation; failure propagates without negative caching. | Resolved by 9a55b6c (PR #15 rider). Migrated to `runTest` + `testScheduler.advanceUntilIdle()` per the UD-807 pattern. |
| `RequestIdPluginTest.kt` | `every request carries X-Unidrive-Request-Id header` | impl-anchored (minor) | Every request carries a non-blank, unique `X-Unidrive-Request-Id`. | Resolved by 6556992. Renamed to `every request carries a non-blank unique X-Unidrive-Request-Id header`; `assertEquals(8, id.length)` impl-anchor dropped. |
| `RequestIdPluginTest.kt` | `request attribute exposes the id for caller introspection` | impl-anchored / partially-vacuous | Builder-side `requestId()` returns the same id that the engine receives in the header. | Resolved by 6556992. Renamed to `RequestId plugin populates X-Unidrive-Request-Id before the request reaches the engine`; the always-null `capturedFromBuilder` capture is gone (Ktor builder lambda runs before plugin `onRequest`). |
