...
|---|---|---|---|---|
| `:app:core` | 2026-05-13 | 9 | 3 | UD-810, UD-811 |
| `:app:sync` | — | — | — | — |
| `:app:cli` | — | — | — | — |
| `:app:benchmark` | — | — | — | — |
| `:app:e2e-360` | — | — | — | — |
...
| `ProviderMetadataTest.kt` | `allByTier returns sorted metadata` | misleading | `allByTier()` returns metadata sorted by the canonical tier order. | Filed UD-810. Body asserted only non-blank tier; name promised sort verification. |
| `ProviderMetadataTest.kt` | `allMetadata returns list` | misleading | Every discovered metadata has non-blank id and displayName. | Filed UD-810. Body's for-loop runs 0 iterations on empty list, comment "may be empty" excuses it — passes when SPI is broken. |
| `ProviderMetadataTest.kt` | `ProviderMetadata stores all required fields` / `optional fields` / `data class equality and copy` | delete-candidate | Kotlin data-class equals/hashCode/copy contract. | Filed UD-810. Tests the compiler, not the domain. Defer until UD-811 clears the impl-anchored cases first. |
| `CloudItemTest.kt` | `equal items have equal hashCodes` / `items differing in any field are not equal` / `hashCode is stable across calls` / `works correctly as HashMap key` | delete-candidate | Kotlin data-class equals/hashCode contract. | Filed UD-810. Same rationale as the ProviderMetadata data-class trio. |
