...
status: closed
closed: 2026-04-29
resolved_by: commit 329c5bb. Investigation complete: every NoSuchMethodError is JVM-internal control flow (MethodHandle resolution + array clinit probes). No dependency drift, no application-level errors. Documented as a lesson so future investigators don't re-investigate. Acceptance criteria #2 and #3 don't apply as written.
opened: 2026-04-21
chunk: core
---
...
status: closed
closed: 2026-04-29
resolved_by: commit b1a5926. Size-adaptive timeout policy in WebDavTimeoutPolicy.kt; default 50 KiB/s min throughput → ~90 min for 277 MB. Long.MAX_VALUE preserved as opt-out (set upload_min_throughput_kbps=0). Download keeps Long.MAX_VALUE (size unknown until response). 7 unit tests including the 2 GiB / 4096 s acceptance fixture.
opened: 2026-04-21
chunk: core
---
...
status: closed
closed: 2026-04-29
resolved_by: commit 75bce60. Rclone audit re-framed as process-shellout (not HTTP). 6 escape failure modes: exit-code collapse (5/6/7→same RcloneException), OAuth-refresh detection regex-dependent, pipe-buffer deadlock latent, stderr discarded on success, no --retries/--transfers passthrough, SIGKILL leaves orphaned multipart IDs + .partial files.
opened: 2026-04-20
chunk: sg5
---
**Part of UD-228 split — per-provider HTTP robustness audit.**
...
status: closed
closed: 2026-04-30
resolved_by: commit d05b6b3. Shutdown hook prints abort summary + logs INFO + deletes tempDir on SIGINT/SIGTERM. cleanStaleTempDirs scans java.io.tmpdir for unidrive-relocate-* > 24h and deletes them silently at every relocate startup. 4 new test cases (delete stale, skip fresh, ignore non-relocate, no-op on missing tmpdir). Pending-transfer state file deferred per ticket scope.
opened: 2026-04-21
chunk: core
---
### Problem
...
status: closed
closed: 2026-04-30
resolved_by: commit 4e5dc71. Out of scope for unidrive-cli (CLI + sync engine + provider adapters only). Extracted to UI_UD.md at repo root with full ticket body preserved. When the UI repo materialises, UI_UD.md moves there.
code_refs:
  - ui/src/main/kotlin/org/krost/unidrive/ui/TrayManager.kt
opened: 2026-04-19
chunk: ipc-ui
...
status: closed
closed: 2026-04-30
resolved_by: commit 41d305a. sync_root persisted to state.db on first run; subsequent runs compare and abort if changed; --reset wipes via db.resetAll() so next run records the new root; case-insensitive on Windows; 4 unit tests added
opened: 2026-04-30
---
...
status: closed
closed: 2026-04-30
resolved_by: commit 132783e. truncateForWidth helper anchors on ' /' to preserve [N/M] verb prefix; cols from COLUMNS env, default 80; 5 unit tests for boundary cases
opened: 2026-04-30
---
...
status: closed
closed: 2026-04-30
resolved_by: commit 2b1d10f. visitFileFailed override logs+continues; lastScanSkipped exposed; baseline + POSIX-only test added (Windows skip via assumeFalse)
opened: 2026-04-30
---
...
status: closed
closed: 2026-04-30
resolved_by: commit f0d79cf. Mirrored UD-329 OneDrive fix: prepareGet().execute() + bodyAsChannel() in HiDriveApiService.downloadFile. No mock-3GB test (canonical pattern, covered by GraphApiService tests).
opened: 2026-04-29
---
...
status: closed
closed: 2026-04-30
resolved_by: commit 8af234e. syncPathSegment appended to UD-296 banner conditionally — silent when unset, ' sync_path=<value>' between sync_root and mode when set. No new test (UD-296 baseline also untested; one-line obvious correctness).
opened: 2026-04-30
---
...
status: closed
closed: 2026-04-30
resolved_by: commit cbfe70f. retryOnTransient + parseRetryAfter helpers added; createFolder/createFile/moveFile/moveFolder wrapped. Tests cover Cloudflare body parsing, single-retry success, non-transient passthrough, budget exhaustion. UD-330 still covers structural cross-provider HttpRetryBudget lift; this is the tactical narrow fix for the user's 502 mkdir cluster.
opened: 2026-04-30
---
...
---
id: UD-748
title: UD-744 slice 2: count-aware ETA refinement (UD-747 follow-up)
category: tooling
priority: medium
effort: S
status: closed
closed: 2026-04-30
resolved_by: commit bd1bff8. Count-aware ETA: progressFraction = currentCount/lastCount → estimatedTotalSec → remainingSecs. 5% progress threshold + 4× sanity clamp guard against bad extrapolation. Falls back to UD-747 wall-clock path when missing/unstable. Per-provider absolute count probes remain in UD-744.
opened: 2026-04-30
---
**UD-744 second slice — count-aware ETA refinement on top of UD-747.**

UD-747 ships an ETA derived from wall-clock subtraction:
`etaSecs = max(0, lastSecs - elapsedSecs)`. Robust but blind to
progress speed: a sync that's 80% done at 50% of last run's time
still reports an ETA based on *time elapsed*, not *items observed*.

This sub-ticket persists the previous run's **final scan count**
per phase and uses progress fraction to refine the bucket:

    progressFraction  = currentCount.toDouble() / lastFinalCount
    estimatedTotalSec = elapsedSecs / progressFraction
    remainingSecs     = max(0, estimatedTotalSec - elapsedSecs)

Falls back to UD-747's wall-clock-only path when count data is
unavailable, the progress fraction is too unstable (< 5%), or the
extrapolation diverges absurdly from the historical seconds (sanity
clamp).

## Where

- `core/app/sync/src/main/kotlin/org/krost/unidrive/sync/SyncEngine.kt`
  — alongside `last_scan_secs_*` persistence, also write
  `last_scan_count_local` / `last_scan_count_remote` after each
  phase. Pre-phase, fire `reporter.onScanCountHint(phase, lastCount)`.
- `core/app/sync/src/main/kotlin/org/krost/unidrive/sync/ProgressReporter.kt`
  — add `onScanCountHint(phase: String, lastCount: Int) {}` (default
  no-op).
- `core/app/cli/src/main/kotlin/org/krost/unidrive/cli/CliProgressReporter.kt`
  — extend `formatEtaBucket` (or add a sibling
  `formatEtaBucketCountAware`) that prefers progress-based ETA when
  count hint exists and progress fraction is ≥ 5%.

## Acceptance

- Heartbeat ETA bucket adapts to in-run speed when count hint
  exists.
- Wall-clock-only ETA still works (UD-747 regression intact).
- Sanity clamp: extrapolated ETA is reduced to historical-time-only
  if it diverges by > 4× from `lastSecs - elapsedSecs`.
- Tests:
  - count-aware bucket boundaries
  - fallback path when no count hint
  - sanity clamp engages when extrapolation is wild

## Out of scope

- Per-provider absolute total-count probe (Graph `$count=true`,
  Internxt drive metadata) — remains in UD-744 parent. The "count"
  here is the previous run's final observation, not a fresh probe.

## Priority / effort

**Medium priority, S effort.** Pure additive refinement; no
schema change (sync_state is k/v); no provider work.

## Related

- **UD-747** (sibling, closed) — wall-clock-only ETA foundation.
- **UD-744** (parent) — keeps the per-provider count probe scope.
- **UD-713** (sibling, closed) — bucket layout.

---
id: UD-749
title: Investigate :app:cli:test hang (likely CliSmokeTest + UD-746 IPC)
category: tooling
priority: medium
effort: S
status: closed
closed: 2026-04-30
resolved_by: commit 0fe719c. Root cause: UD-738 test invoked cli.execute() without -c isolation, picked up developer's default profile, ran real 2228-action sync. Fix: switch to cli.parseArgs() which returns ParseResult without invoking run(). Both --reset and --dry-run appear in matchedOptions() iff no ParameterException — that's the actual assertion.
opened: 2026-04-30
---
**Test-infra issue surfaced during UD-746 follow-up cleanup
(2026-04-30).**

Running `./gradlew :app:cli:test` hangs indefinitely on the test
phase after the UD-746 / UD-336 / UD-747 / UD-748 commit chain.
The build progresses through compile / classes / jar fine, then
stops at `> Task :app:cli:test` with no further output. PID stays
alive for 25+ minutes; manual SIGKILL is the only way out.

**Pattern observed:** the test discovery succeeds — Gradle hands
control to the JUnit runner. The first test class
(`CliProgressReporterTest`, alphabetical order) completes and
emits its XML. The next class (`CliSmokeTest` by alphabetical
order) appears to hang.

**Suspect:** `CliSmokeTest` spawns sub-processes via
`ProcessBuilder("java", "-jar", jarPath, ...)`. Each subprocess
now creates an `IpcServer` socket because UD-746 made IPC
unconditional in `SyncCommand`. If the subprocess invokes
`unidrive sync` (or any subcommand that touches IPC) and the
test doesn't fully drain its stdout before joining, the AF_UNIX
/ Named-Pipe handle may not be released cleanly on Windows,
blocking the test fork.

The test was passing on UD-746's own commit (`4a69fc7`) per its
build log (`BUILD SUCCESSFUL in 32s`), so the regression must
have been introduced by a subsequent change OR by something
about test ordering / classpath state.

**Workaround in use:** I've been running individual test classes
(e.g. `--tests CliProgressReporterTest`) which complete fine
because the fork doesn't hit `CliSmokeTest`.

## Diagnostic next steps

1. Bisect: run `git log --oneline UD-746..HEAD` and re-run
   `:app:cli:test` at each commit to identify the regression.
2. Check `CliSmokeTest`'s test methods one by one (`--tests
   "*CliSmokeTest.version output format"` etc.) to see which
   specific subprocess hangs.
3. Inspect socket file at `IpcServer.defaultSocketPath()` after
   a hung run — is it still locked / open? Stale sockets may need
   cleanup.
4. Check whether `ProcessBuilder.start().waitFor(timeout)` would
   surface the issue more cleanly than the current pattern.

## Where

- `core/app/cli/src/test/kotlin/org/krost/unidrive/cli/CliSmokeTest.kt`
  — primary suspect; spawn pattern + stdout draining.
- `core/app/sync/src/main/kotlin/org/krost/unidrive/sync/IpcServer.kt`
  — socket lifecycle on Windows.
- `core/app/cli/src/main/kotlin/org/krost/unidrive/cli/SyncCommand.kt`
  — UD-746 unconditional IPC creation.

## Acceptance

- `./gradlew :app:cli:test` completes within 5 minutes (warm
  daemon).
- `CliSmokeTest` either completes its checks or skips cleanly
  (`assumeTrue(jarPath.exists())` already partial — but the hang
  may be later in the class).
- Root cause documented in this ticket OR upstream lesson note
  (`docs/dev/lessons/`).

## Priority / effort

**Medium priority, S effort.** Doesn't block compile / unit-test
runs of individual classes, but blocks any "run all tests" CI
gate. Likely a 1-2 hour bisect + targeted fix.

## Related

- **UD-746** (closed) — likely root cause of the regression
  (unconditional IPC).
...
---
id: UD-340
title: Lift UD-333 HTML body-sniff guard to :app:core/http
category: providers
priority: high
effort: XS
status: closed
closed: 2026-04-30
resolved_by: commit 6720cf3. Shared assertNotHtml at :app:core/http (UD-333 lineage). OneDrive/HiDrive/Internxt all consume; Internxt-specific silent-file-corruption risk closed across all three.
opened: 2026-04-30
---
**From the 2026-04-30 provider-duplication survey.**
...
---
id: UD-343
title: Lift `Json { ignoreUnknownKeys; isLenient }` config to :app:core/SharedJson
category: providers
priority: low
effort: XS
status: closed
closed: 2026-04-30
resolved_by: commit b3f5755. Bundled XS lift from 2026-04-30 survey. See commit b3f5755 for full prose.
opened: 2026-04-30
---
**From the 2026-04-30 provider-duplication survey.**
...
---
id: UD-347
title: Lift `setPosixPermissionsIfSupported` to :app:core/io
category: providers
priority: low
effort: XS
status: closed
...
---
id: UD-351
title: Lift OAuth PKCE helpers (codeVerifier + codeChallenge) to :app:core/auth
category: providers
priority: low
effort: XS
status: closed
...
opened: 2026-04-30
---
**From the 2026-04-30 provider-duplication survey. Filed manually
as the comparison case for the backlog.py tool — same agent finding,
hand-composed frontmatter to test the human-vs-tool delta.**

Word-for-word duplicate PKCE helpers in two providers' OAuthService:
32-byte SecureRandom, Base64.urlEncoder.withoutPadding, SHA-256
challenge.

- core/providers/onedrive/.../OAuthService.kt:114-122
- core/providers/hidrive/.../OAuthService.kt:65-83

## Proposal

`:app:core/auth/Pkce.kt`:
...
Both providers' tests verify the exact `MessageDigest.getInstance("SHA-256")`
output via `java.util.Base64.getUrlEncoder().withoutPadding()` — re-test
the lifted helper once, drop the duplicate test logic.

## Acceptance

- OneDrive + HiDrive consume the shared `Pkce` object.
- Pkce challenge round-trip test fixture matches RFC 7636 vector.
...
- **3.4 oauth-loopback-callback-server** (sibling, UD-348) — same
  package destination.
...
---
id: UD-346
title: Lift snapshot-delta diff loop to :app:sync/SnapshotDeltaEngine
category: providers
priority: medium
effort: M
status: closed
closed: 2026-05-01
resolved_by: commit cf4ce36. Shared computeSnapshotDelta in :app:sync; six provider delta() impls shrunk to ~20 lines each.
opened: 2026-04-30
---
**From the 2026-04-30 provider-duplication survey.**
...
status: closed
closed: 2026-05-01
resolved_by: commit 4f0f22c. LocalScanner upserts pending-upload row + Reconciler synthesises retry-upload + RemoveEntry-on-deleted-pending paths
opened: 2026-05-01
---
...
status: closed
closed: 2026-05-01
resolved_by: commit 2f32af6. LocalScanner.isSparseLeftover guard + Reconciler pending-upload-NEW special case
opened: 2026-05-01
---
...
status: closed
closed: 2026-05-01
resolved_by: commit 8a6d3ad. test passes maxConcurrentTransfers=1 for deterministic CE-vs-sibling ordering
opened: 2026-05-01
---
**Surfaced 2026-05-01 on sg5 during `./gradlew build`.**
...
status: closed
closed: 2026-05-01
resolved_by: commit ccf888e. SPECS \u00a70 + \u00a74.1, PlaceholderManager.kt:40-46 KDoc, CHANGELOG bullets all updated
opened: 2026-05-01
---
**Surfaced 2026-05-01 on sg5 during `./gradlew build`.**
...
---
id: UD-407
title: CLI ignores UNIDRIVE_CONFIG_DIR env var (MSIX config drift root cause)
category: cli
priority: high
effort: S
status: closed
closed: 2026-05-03
resolved_by: commit 8aa9dfd. CLI now reads UNIDRIVE_CONFIG_DIR (parity with MCP). Canonical SyncConfig.defaultConfigDir() centralises the precedence: explicit flag > env var > XDG-when-exists > APPDATA > XDG-fallback. CLI's baseConfigDir delegates. Pure resolveConfigDir() variant unit-tested across 6 precedence cases. MSIX diagnostic gains a 'move to ~/.config/unidrive/' option.
code_refs:
  - core/app/cli/src/main/kotlin/org/krost/unidrive/cli/Main.kt:80
  - core/app/sync/src/main/kotlin/org/krost/unidrive/sync/SyncConfig.kt:291
  - core/app/mcp/src/main/kotlin/org/krost/unidrive/mcp/Main.kt:76
opened: 2026-05-03
---
**The CLI advertises `UNIDRIVE_CONFIG_DIR` as the workaround for MSIX-virtualised `%APPDATA%` ([Main.kt:791](core/app/cli/src/main/kotlin/org/krost/unidrive/cli/Main.kt:791)) but never reads the variable.** Only the MCP entry point honours it ([app/mcp/Main.kt:76](core/app/mcp/src/main/kotlin/org/krost/unidrive/mcp/Main.kt:76)). The CLI's `baseConfigDir` getter and `SyncConfig.defaultConfigDir()` both fall straight through to `APPDATA`, so users who set the env var as instructed see *no behaviour change* — and config drift between MSIX-sandboxed Claude Desktop reads and native-shell PowerShell reads of the same nominal path silently corrupts state.

Concrete incident (2026-05-03): same `unidrive -p inxt_gernot_krost_posteo` invocation resolved to two different `sync_root` values between a native PowerShell run (`C:\Users\gerno\InternxtDrive`) and a Claude-Desktop-spawned run (`C:\Users\gerno\unidrive-internxt-gernot`) because the two processes read different on-disk files at the same nominal `C:\Users\gerno\AppData\Roaming\unidrive\config.toml` path.

## What to change

1. **[app/cli/Main.kt:80–89](core/app/cli/src/main/kotlin/org/krost/unidrive/cli/Main.kt:80)** — `baseConfigDir` getter: prepend an `UNIDRIVE_CONFIG_DIR` check before the `APPDATA`/`HOME` fallback. Mirrors `app/mcp/Main.kt:76` so the two entry points behave identically.

2. **[app/sync/SyncConfig.kt:291–298](core/app/sync/src/main/kotlin/org/krost/unidrive/sync/SyncConfig.kt:291)** — `defaultConfigDir()` companion: same prepend. Several callers in the engine and tools resolve via this static path; without it set, profile auto-detection still drifts.

3. **Optional but cheap**: when both the env var AND the explicit `-c/--config-dir` flag are unset AND `~/.config/unidrive/config.toml` exists, prefer the XDG path over `%APPDATA%`. This makes Linux/Mac/Windows-MSIX users converge on the same default with zero env-var setup once they've copied their config there.

4. Add a unit test asserting that:
   - `UNIDRIVE_CONFIG_DIR=/some/path` overrides `APPDATA`
   - In its absence, behaviour is unchanged
   - The XDG-prefer fallback applies only when `~/.config/unidrive/config.toml` exists

## Acceptance

- `UNIDRIVE_CONFIG_DIR=C:\Users\gerno\.unidrive` set at system level makes `unidrive -p inxt_gernot_krost_posteo` resolve identically from native PowerShell and from a Claude-Desktop-spawned subprocess.
- No regression for users who haven't set the env var.
- Test coverage pins the precedence order: `--config-dir` flag > `UNIDRIVE_CONFIG_DIR` > `XDG path if exists` > `%APPDATA%` > `HOME/.config/unidrive`.

## Related

- [Claude Desktop MSIX sandbox](memory) — Claude's process tree sees virtualised `%APPDATA%` while user's native shell sees real NTFS at the same path.
- [docs/dev/lessons/](docs/dev/lessons/) — should NOT add a new lesson for this; that's the failure mode the ticket is correcting.

---
id: UD-225a
title: Reconciler emits DeleteRemote for unhydrated rows when local stub missing
category: core
priority: critical
effort: S
status: closed
closed: 2026-05-03
resolved_by: commit f9f1791. Reconciler.resolveAction's DELETED+UNCHANGED arm now emits DownloadContent for unhydrated file rows (synthesised CloudItem from DB if delta doesn't carry it), preserves DeleteRemote for hydrated rows. 3 new tests + 6 existing tests updated to use isHydrated=true where they posit user-delete semantics. Live dry-run on the 1,550-row /_INBOX scenario: would download 1,426, upload 0 (was 0/0 pre-fix).
code_refs:
  - core/app/sync/src/main/kotlin/org/krost/unidrive/sync/Reconciler.kt:352
  - core/app/sync/src/main/kotlin/org/krost/unidrive/sync/LocalScanner.kt:147
opened: 2026-05-03
---
**`Reconciler.resolveAction` emits `DeleteRemote` instead of `DownloadContent` when a placeholder DB entry is unhydrated and the local stub is missing.** Same bug class as UD-225 (non-hydrated DB entries getting wrong reconciler action) but different code path. UD-225 patched the `UNCHANGED+UNCHANGED` skip via a recovery loop; this ticket patches the `DELETED+UNCHANGED` decision in the main loop.

## Concrete incident (2026-05-03)

User runs `unidrive -p inxt_gernot_krost_posteo sync --download-only --sync-path=/_INBOX` against a sync_root with no `_INBOX/` subdirectory. Expected: ~1,550 files (~6.3 GB) downloaded from remote into a fresh `_INBOX/`. Observed: `Reconciled: 0 actions`. State.db has 1,550 entries under `/_INBOX` (all `is_hydrated=0`, `remote_id` set, populated during a 2026-05-03T11:01 incident sync that emitted CreatePlaceholder for the directory tree but never ran the hydration phase before the user cancelled).

## Mechanism

For each `/_INBOX/*.pdf` path:

1. **LocalScanner** at [LocalScanner.kt:147](core/app/sync/src/main/kotlin/org/krost/unidrive/sync/LocalScanner.kt:147) sees DB entry but no file → emits `localChanges[path] = ChangeState.DELETED`.
2. **Reconciler main loop** at [Reconciler.kt:100](core/app/sync/src/main/kotlin/org/krost/unidrive/sync/Reconciler.kt:100) computes `remoteState=UNCHANGED` (file present in delta, `hash`+`modified` match DB).
3. **`resolveAction`** at [Reconciler.kt:352-360](core/app/sync/src/main/kotlin/org/krost/unidrive/sync/Reconciler.kt:352) returns `SyncAction.DeleteRemote(path)` — assuming user-initiated local delete that should propagate.
4. **UD-225 recovery loop** at [Reconciler.kt:185](core/app/sync/src/main/kotlin/org/krost/unidrive/sync/Reconciler.kt:185) skips because `entry.path in coveredPaths` is true (DeleteRemote occupies the slot).
5. **Engine direction filter** at [SyncEngine.kt:232-258](core/app/sync/src/main/kotlin/org/krost/unidrive/sync/SyncEngine.kt:232) drops `DeleteRemote` in `--download-only` mode → 0 actions emitted, files unreachable forever until the user manually clears the DB or fixes the bug.

In bidirectional mode the bug is even worse: the user gets 1,426 actual `DeleteRemote` calls executed, **destroying remote data** based on a false interpretation of "local is missing." `is_hydrated=0` means "this row represents a planned-but-unfulfilled local presence" — the local-missing state IS the original state, not a state change.

## What to change

In [`Reconciler.kt:352`](core/app/sync/src/main/kotlin/org/krost/unidrive/sync/Reconciler.kt:352), the `localState=DELETED + remoteState=UNCHANGED` branch should add a third clause for the unhydrated case:

```kotlin
localState == ChangeState.DELETED && remoteState == ChangeState.UNCHANGED ->
    when {
        // UD-901: pending-upload row with no remote yet → just drop the placeholder
        entry != null && entry.remoteId == null -> SyncAction.RemoveEntry(path)
        // UD-225a: unhydrated placeholder where local was never created →
        // re-hydrate, don't delete remote. The local-missing state IS the
        // original state; absence is not user intent.
...
- `unidrive sync --download-only --sync-path=/_INBOX` against a sync_root missing `_INBOX/` but with 1,550 unhydrated DB rows pointing at remote files in `/_INBOX/` emits 1,426 DownloadContent actions (one per file).
- Same scenario in bidirectional mode does NOT emit DeleteRemote for unhydrated rows.
- Unit test in `ReconcilerTest`: seed an unhydrated entry (`isHydrated=false, remoteId=set, remoteSize>0`), pass `localChanges[path]=DELETED, remoteChanges[path]=<unchanged item>`, assert action is `DownloadContent`, not `DeleteRemote`.
- Regression test: existing "user genuinely deleted local + bidirectional sync → DeleteRemote" pin still passes (the `entry.isHydrated=true` path is unchanged).

## Related

- UD-225 (closed) — sibling fix in the recovery loop after the main loop.
- UD-222 — "files always hydrate" mandate that the reconciler is supposed to honour.
- UD-209b — sparse-file leftovers as another source of `isHydrated=false` rows.

---
id: UD-225b
title: Engine should use downloadById, not path-based download, when action carries remoteId
category: core
priority: high
effort: S
status: closed
closed: 2026-05-03
resolved_by: commit 2dac299. Engine's 4 download dispatch sites (applyCreatePlaceholder, applyUpdatePlaceholder, applyDownload, conflict-resolution) now route through a single downloadByIdOrPath helper that prefers provider.downloadById when the action's CloudItem has a non-empty id, falls back to path-based for empty id. Eliminates per-segment folder traversal for Internxt/OneDrive Graph; default delegation keeps other providers unchanged. 2 new SyncEngineTest cases pin both dispatch branches.
code_refs:
  - core/app/sync/src/main/kotlin/org/krost/unidrive/sync/SyncEngine.kt:781
  - core/app/sync/src/main/kotlin/org/krost/unidrive/sync/SyncEngine.kt:802
  - core/app/sync/src/main/kotlin/org/krost/unidrive/sync/SyncEngine.kt:823
  - core/app/sync/src/main/kotlin/org/krost/unidrive/sync/SyncEngine.kt:1004
  - core/app/core/src/main/kotlin/org/krost/unidrive/CloudProvider.kt:36
opened: 2026-05-03
---
**Engine's `applyDownload` / `applyCreatePlaceholder` / `applyUpdatePlaceholder` / conflict-resolution path all call `provider.download(remotePath, dest)` even though the action carries a `CloudItem` with a valid `remoteId`.** Path-based download forces the provider to re-traverse the folder tree by name to resolve the file (`InternxtProvider.resolveFolder` walks segment-by-segment via `getFolderContents` per parent). That's:

1. **Wasteful**: O(depth) round-trips per file. For 1,426 files in /_INBOX with avg depth 4, that's ~5,700 extra round-trips at ~500 ms each ≈ 47 minutes of pointless API traffic.

2. **Fragile**: any folder whose name isn't matched (UD-317 sanitization edge case, encoding/case mismatch, transient `getFolderContents` 503) fails the WHOLE download with `ProviderException: Folder not found: <segment> in <path>`. Live 2026-05-03: ~44 % failure rate (113 fails out of 258 attempted in 3 min) on the user's UD-225a recovery sync because some intermediate folder names couldn't be resolved.

3. **Redundant**: the action's `remoteItem.id` is the file UUID. `InternxtProvider.downloadById` uses it to fetch directly from the Bridge API — no folder traversal needed.

The default `CloudProvider.downloadById` falls back to `download(remotePath, destination)` for providers that don't override (line 36-40), so this change is safe across the SPI: providers that have a real id-based path (Internxt, OneDrive Graph) take the fast/robust path; others get the existing semantics.

## What to change

In `core/app/sync/src/main/kotlin/org/krost/unidrive/sync/SyncEngine.kt`, replace `provider.download(...)` with `provider.downloadById(item.id, path, dest)` at the four call sites:

- `applyCreatePlaceholder` ([SyncEngine.kt:781](core/app/sync/src/main/kotlin/org/krost/unidrive/sync/SyncEngine.kt:781))
- `applyUpdatePlaceholder` ([SyncEngine.kt:802](core/app/sync/src/main/kotlin/org/krost/unidrive/sync/SyncEngine.kt:802))
- `applyDownload` ([SyncEngine.kt:823](core/app/sync/src/main/kotlin/org/krost/unidrive/sync/SyncEngine.kt:823))
- conflict-resolution `download remote into .conflict-remote-*` ([SyncEngine.kt:1004](core/app/sync/src/main/kotlin/org/krost/unidrive/sync/SyncEngine.kt:1004))

All four already have the `CloudItem` in scope (action.remoteItem or local `item` variable). Edge case: if `remoteItem.id` is empty (synthesized CloudItem from DB with `entry.remoteId == null` — shouldn't happen post-UD-225a's fallback to `entry.remoteId ?: ""`, but defensible), fall through to the existing path-based `download` for safety.

## Acceptance

- The user's UD-225a recovery sync (`unidrive sync --download-only --sync-path=/_INBOX` with 1,426 unhydrated rows) completes with **zero `Folder not found` errors** under normal-Internxt conditions.
- Wall-clock for the 1,426 / ~6.3 GB download drops from "8–14 hr with ~50 % failure" to roughly "shard-throughput-bound" (UD-353 floor: 30–50 KB/s × 6.3 GB ≈ 35–58 hr in worst case, or 1–2 hr if Internxt isn't OVH-throttled). The arithmetic shifts from "API traversal cost" to "shard byte transfer".
- Existing path-based download still works for providers that don't override `downloadById` (the default delegates).
- Test: a `SyncEngineTest` case asserts `applyDownload` calls `provider.downloadById(item.id, ...)` rather than `provider.download(path, ...)` when `remoteItem.id` is non-empty.

## Related

- UD-225a (closed) — sibling fix that unlocked the 1,426 download actions UD-225b is now servicing.
...
status: closed
closed: 2026-05-03
resolved_by: commit 47320d1. Closed as invalid after Phase 0 reconnaissance. /notifications is an account-level marketing/promotional channel — schema NotificationWithStatusDto carries link/message/expiresAt only, with example payload 'Black Friday Sale - 50% off all plans!'. No file UUIDs, no event types, no resource paths. Internxt publishes no public real-time change feed; the cost-of-delta() debate remains the right path forward (see UD-362, UD-363). Phase 0 report at docs/audits/internxt-notifications-feasibility.md; audit-doc capability matrix line 148 updated.
code_refs:
  - core/providers/internxt/src/main/kotlin/org/krost/unidrive/internxt/InternxtProvider.kt
  - core/app/core/src/main/kotlin/org/krost/unidrive/CloudProvider.kt
  - docs/audits/internxt-api-vs-spi.md
...
status: closed
closed: 2026-05-03
resolved_by: commit 47a1f4e. main() now calls sys.stdout.reconfigure(encoding='utf-8', errors='replace') (and same for stderr) when the current encoding isn't already UTF-8. Verified on Windows PowerShell against %LOCALAPPDATA%\unidrive\unidrive.log without PYTHONIOENCODING. No behaviour change on Linux (already UTF-8).
code_refs:
...
status: closed
closed: 2026-05-03
resolved_by: commit 5cf294d. RequestIdPlugin.onResponse appends 'up=N dn=M' (or '?' for streaming) after the existing '(Nms)' chunk; suppressed when both counts are zero or both null. Goes after the ms close-paren so log-stats.py's r'\(\d+\)ms\)' regex (line 35) keeps matching unchanged. Three tests cover suppress/emit/streaming behaviour. log-throughput.py consumer was referenced in the ticket but the file doesn't exist in the tree — left as a follow-on if/when that script lands.
code_refs:
