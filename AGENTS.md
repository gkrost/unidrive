...
- **Doc surface is bounded.** Shared docs are this file, `README.md`, `BACKLOG.md`, `CLOSED.md`, `BOOSTERS.md`, ADRs under `docs/adr/`, audits under `docs/audits/`. Per-module `README.md` files inside `core/providers/<name>/` and `core/app/<module>/` are permitted; they document that module only. New ADRs may be added when a decision is load-bearing enough to outlive memory. New audits when a provider quirk needs reverse-engineering.
