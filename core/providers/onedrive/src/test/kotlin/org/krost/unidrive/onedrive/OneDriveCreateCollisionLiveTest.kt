...
/**
 * UD-366 LIVE data-loss drive (OneDrive) — Unit 1 acceptance.
 *
 * Drives the exact silent-overwrite mechanism against the real `posteo_onedrive` account:
 ...
 * copy is kept-both under a different (server-renamed) path.
 *
 * If this test is removed or loosened, the create-collision blind-overwrite can silently regress.
 *
 * Gated: requires UNIDRIVE_INTEGRATION_TESTS=true and the posteo_onedrive token.
 */
class OneDriveCreateCollisionLiveTest {
    private val profile = System.getenv("UNIDRIVE_LIVE_ONEDRIVE_PROFILE") ?: "posteo_onedrive"
    ...
    private fun enabled(): Boolean {
        ...
        return Files.exists(tokenPath.resolve("token.json"))
    }

    @Test
    fun `create collision preserves the pre-existing remote and keeps both`() {
        if (!enabled()) return
        val provider = OneDriveProvider(OneDriveConfig(tokenPath = tokenPath))
        runBlocking {
            ...
            try {
                ...
                assertNotEquals(
                    ...
                    "keep-both must land the local copy under a renamed path, not overwrite the original",
                )
