...
| Pre-reconcile empty-sync_root guard must differentiate "rehydrate intent" from "wrong sync_root" | `SyncEngine.kt:540-552` fires when state.db has hydrated rows but `sync_root` is empty, and recommends `--force-delete`. That recommendation is the bidirectional override and semantically wrong for a `--download-only` user who wiped local intentionally and wants the cloud copy back. Options: (a) new `--rehydrate-from-cloud` flag that asserts intent and skips the guard; (b) detect `--download-only` in the guard and auto-skip (destructive direction is already gated); (c) make the message conditional on the direction filter. Without this, the documented escape hatch points at the wrong action class. Companion cleanup: `SyncCommand.kt:424` calls `Files.createDirectories(syncRoot)` BEFORE the guard runs, so even an aborted run leaves an empty sync_root behind — move the create AFTER the guard. |
