...
| `unidrive status --all` triggers interactive auth + shows only one profile | With two Internxt profiles configured (e.g. `internxt` and `gernot_krost_internxt_pst`), `unidrive status` shows the first; `unidrive status --all` shows ONLY the second AND triggers an interactive email/password/2FA prompt during status display. Two distinct bugs: (a) `--all` should enumerate ALL configured profiles, not just one; (b) profile auth should happen via `unidrive auth`, not as a side effect of `status` (status is supposed to be a fast, side-effect-free read). |
