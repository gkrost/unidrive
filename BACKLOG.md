...
| Title | Scope |
|---|---|
| Internxt delta path-collapse (phantom folders) | `InternxtProvider.buildFolderPath` (around `InternxtProvider.kt:569-580`) returns `""` when a parent uuid is missing from the in-memory `folderMap`, silently routing items to the root and orphaning subtrees. Fix: drop the item, mark the snapshot incomplete, do not emit a false local-delete. Tracked in [docs/audits/internxt-phantom-investigation.md](docs/audits/internxt-phantom-investigation.md). |
| Internxt state-db duplicate-`remote_id` cleanup | One-shot migration / `unidrive repair` to drop the duplicate rows already produced by the phantom-folder bug. Must land alongside or after the path-collapse fix. |
...
| Internxt `status` filter selectable per call | Hard-coded `status=ALL` at `InternxtApiService.kt:50`. |
