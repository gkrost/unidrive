...
| Unhydrated folder rows must not turn into DeleteRemote | `LocalScanner.scan()` (`LocalScanner.kt:153-162`) emits `ChangeState.DELETED` for every DB row whose path is not on disk, with no `isHydrated` filter; the Reconciler's `(local=DELETED, remote=UNCHANGED)` recovery downgrade applies only to files (`!entry.isFolder` at `Reconciler.kt:630`), so unhydrated folder rows fall through to `SyncAction.DeleteRemote`. On a sparse-hydration profile this plans deletion of every cloud folder the user has not visited; only the top-level-never-hydrated guard plus the bulk-delete safeguard currently stand between the engine and large-scale cloud loss. Fix at either end (filter `isHydrated=false` rows out of the LocalScanner deletion sweep, or extend the recovery downgrade to folders with a placeholder-recreate / no-op action). Two regression smokes: (a) seed 100 unhydrated folder rows + walk an empty `syncRoot`, assert zero `DeleteRemote` actions; (b) pure-Reconciler test with `localChanges={folder→DELETED}, remoteChanges=∅, entry.isFolder=true, entry.isHydrated=false`, assert no `DeleteRemote`. Same class as the tracking-set entry above — but the legacy engine is still in use and this can't wait for the rewrite. |
