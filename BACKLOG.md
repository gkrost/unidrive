...
| Internxt `HttpRetryBudget` wiring | Budget class exists in `:app:core` but `InternxtApiService` doesn't consume it; today's sequential page-fetch path leaves throughput on the table the moment the gateway's per-call latency rises (live runs see 28–52 s per `/drive/files` page). Wire two per-host budgets, mirroring drive-desktop's `bottleneck` config (audited): **Drive REST `maxConcurrency=2, minSpacing=500ms`** (`driveApiBottleneck` in `src/apps/shared/HttpClient/client.ts`) and **Bridge upload `maxConcurrency=4, minSpacing=0`** (`uploadBottleneck` in `src/apps/main/auth/handlers.ts`). The earlier "1 conc / 1000 ms global" reference was the legacy `schedule-fetch.ts` path, not a global cap. |
| Internxt parallel listing pagination | `InternxtProvider.delta()` issues `listFiles` then `listFolders` sequentially, one page at a time. Under the current 28–52 s per-page gateway latency, a 13k-item account takes ~3 hours just to scan. Refactor to fan out 2 page fetches concurrently (matches the Drive `HttpRetryBudget` cap) and run the files/folders streams in parallel rather than serially. Speculative next-page fetch wastes one request on the last partial page; cheaper than the latency win. Depends on HttpRetryBudget wiring landing first so the fan-out has a safety cap. |
