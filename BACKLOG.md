...
| Streaming reconciliation (process pages as they arrive) | The current `SyncEngine.gatherRemoteChanges()` accumulates the full `Map<String, CloudItem>` before `Reconciler.reconcile()` runs — on a 625 GiB / 171k-file account that means user-facing actions wait 7+ hours. Streaming = per-page reconcile firing safe-now actions (`DownloadContent` / `Upload` / `Create*` / explicit `Move`) immediately while deferring deletion-bearing actions (`DeleteLocal` / inferred `DeleteRemote` / `Conflict` × `DELETED`) to scan-end so the existing `detectMissingAfterFullSync` deletion-safety guard isn't broken. Reuses `Reconciler.reconcile()` unchanged (it's already a pure function over its inputs; slicing happens at the call boundary). Additive `scan_staging.reconciled` column tracks STAGED → RECONCILED state for crash-recovery — no schema_version bump. Bounded channel (capacity 4 pages) for backpressure pauses the scan rather than buffering unbounded. 1-page lookahead buffer absorbs same-uuid renames across adjacent pages. Generalizes to OneDrive via the engine-side wrap with zero SPI changes. Behavior-flagged via `sync.streaming_reconciliation` TOML key (default `true` new install, `false` on upgrade, auto-flip after first successful streaming scan). Spec ready (~700 LoC, L tier, 7-commit split); hard blockers all landed. |
...
| Internxt `encryptVersion` legacy support | Hard-coded `03-aes`; legacy `02-rsa` buckets unreadable. Investigation: `02-rsa` cipher/key-derivation format is not present in any current or historical public Internxt repo (`inxt-js` back to v1.2.0, `sdk`, `drive-web`, `drive-desktop`, `drive-mobile`, `swift-core`, `cli`, `drive-server-wip`, `service-storage-models`, `core`, `node-lib`) — all paths use `aes-256-ctr` unconditionally. `EncryptionVersion` enum in `@internxt/sdk` (`src/drive/storage/types.ts`) only defines `Aes03 = '03-aes'`. The provider's own `UserKeysEncryptVersions` enum (`ecc`/`kyber`) governs account keys, not file content. **Blocker: cipher details not discoverable from public source — guessing would silently decrypt to garbage (worse than current behaviour). Defer until either (a) a real legacy account surfaces with a reproducible test fixture, or (b) Internxt publishes the format.** Acceptable interim: a fail-fast guard that throws `ProviderException("Unsupported encryptVersion: $version")` on anything other than `03-aes` so legacy users get a clear error instead of corrupt bytes — narrow guard-only change, no decryption attempt. |
