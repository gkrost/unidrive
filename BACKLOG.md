...
| Internxt WebSocket change feed (wake-signal integration) | drive-desktop subscribes to `process.env.NOTIFICATIONS_URL` via socket.io and uses event frames as a coarse-grained wake signal — not authoritative content — to trigger their existing checkpoint walk. The full feasibility audit (`docs/audits/internxt-websocket-feasibility.md`) confirms the schema is public on both producer (`drive-server-wip/src/externals/notifications/`) and consumer (`drive-desktop/src/backend/features/remote-notifications/`) sides; only the fan-out service is closed-source. Integration is one `NotificationsClient` (~200-300 LoC connection manager + ~100 LoC event router) emitting a single debounced `RemoteChangeHint` into our existing delta walker, with the `clientId` loopback dropped via a `'drive-desktop-unidrive'` literal on our mutations. Make `NOTIFICATIONS_URL` env-overridable (mirrors `INTERNXT_CLIENT_NAME` / `CRYPTO_KEY` pattern). Periodic full scans remain as the source of truth (drop frequency from "poll cadence" to ~daily safety-net). Sidesteps the 20k-item gateway throttle entirely for steady-state work. Promoted from Deferred because the throttle is a present-day pain. Lands after resumable scan — both share the same delta-walker integration point. |
